...
/* MEquivHash cached vectors */
std::vector<double> __l_sum;
std::vector<double> __r_sum;
std::vector<std::vector<arma::uword>> __comp;
arma::mat __p_cols;
arma::mat __permuted;
}
std::size_t
...
MEquivEqual::operator()(const arma::mat & lhs, const arma::mat & rhs) const {
	...
	if(lhs.n_cols == rhs.n_cols) {
		std::vector<arma::uword> perm;
		perm.reserve(lhs.n_cols);
		compatible_vectors(lhs, rhs, __comp);
		result = check_permutation(lhs, rhs, perm, 0, __comp);
	}
	return result;
}
void
...
MEquivEqual::compatible_vectors(const arma::mat & lhs, const arma::mat & rhs,
	...
	for(arma::uword j = 0, max = lhs.n_cols; j < max; ++j) {
		for(arma::uword i = 0; i < max; ++i) {
			if(double_equal(__l_sum[j], __r_sum[i])) result[j].push_back(i);
		}
	}
}
bool
MEquivEqual::check_permutation(const arma::mat & lhs, const arma::mat & rhs,
	...
	if(index == lhs.n_cols) {
		...
		for(arma::uword i = 0, max = __permuted.size(); i < max && equal; ++i) {
			equal = double_equal(__permuted(i), lhs(i));
		}
		result = equal;
	} else {
